...
- [ ] ライブラリにtrain, favoritesなどをまとめた結果スマホの画面幅を超えました。メニューにも残っているのでライブラリからは切り出して、個別一覧画面とするのがいいかもしれません。いや、直接カルーセルに移行したらいいのかな？となるとヘッダー行に、今traindエアルなどの目印が欲しいです。ぱっと見何を見ているのかの違いがわからないです。Queueもヘッダに示した方がいいかもしれません。色を変える方がパッと見てわかりやすいかも
- [ ] カテゴリ登録画面の名前, 説明入力欄に文字を入力しても背景色と同じ色で見えません(ダークモードだから？)
- [ ] メニューの、左右切替メニューを押しても何も変わらないです
- [ ] discoverで検索して見つかった記事を選ぶと真っ白な画面に飛びます。エラーは下記
```
...
```
- [ ] legacyの設定画面にある feed url extractor, open rss, genfeedのリンクはこちらの設定画面にも同様に追加してください
- [ ] 設定のエクスポートインポートにも対応してください。JSONでダウンロード、アップロードを想定。フィードURL郡の出力と、嗜好ベクトルの出力をイメージしています
- [ ] ヘッダーの矢印は不要です手のアイコンがついたので
- [ ] ライブラリのreadタブにunreadボタンは不要です。開いたら変更できるので
- [ ] ノートをつけているかどうかも記事を開いた時、一覧でのアイコンともにわかるようにしたい
- [ ] ノートの編集削除アクションがヘッダにある。アクションはボトムがきほん
- [ ] adminパネルのサービス一覧のラベルが黒い文字で読めない(ダークモードの時)
- [ ] adminパネルのrefreshボタンがヘッダにある。操作ようボタンは全てボトム
- [ ] 戻るボタンとメニューボタン(ボトムから少し浮いたボタン)はもう少し目立たせたい
- [ ] 全体的に、インプットフォームなどがダークモードの時に黒い文字で読めない
