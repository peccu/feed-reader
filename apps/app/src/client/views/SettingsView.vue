<template>
  <div class="h-full flex flex-col bg-background">
    ...
    >
      ...
      <h1 class="text-base font-semibold">設定 / フィード管理</h1>
    </div>

    <div class="flex-1 overflow-y-auto p-4 space-y-6">
      <!-- Add feed -->
      <section>
        <h2 class="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">フィード追加</h2>
      ...
      <section>
        <h2 class="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">
          登録フィード ({{ feedsStore.feeds.length }})
        </h2>
        <div class="space-y-2">
          <div
          ...
          >
            <div class="flex-1 min-w-0">
              ...
              <p v-if="feed.lastPolledAt" class="text-xs text-muted-foreground/60">
                最終取得: {{ new Date(feed.lastPolledAt).toLocaleString('ja-JP') }}
              </p>
            </div>
      ...
      <!-- URL ingest -->
      <section>
        <h2 class="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">URL 投稿</h2>
        <div class="flex gap-2">
          <input
            v-model="ingestUrl"
            type="url"
...
async function ingestArticle() {
  ...
  ingestMessage.value = `ジョブ登録: ${res.jobId}`;
  ingestUrl.value = "";
  setTimeout(() => {
    ingestMessage.value = "";
  }, 3000);
